...
use crate::style::{ButtonStyle, CardStyle, ScrollableStyle, TextStyle};
...
pub fn preview(search: &SearchContext, info: &GenericPost, handle: Handle) -> Element<'static> {
	...
	let image: Element = Scrollable::new(image).style(ScrollableStyle).into();
			...
			chunk
				...
				.map(|tag| {
					if search.exclude.contains(tag) {
						Button::new(Text::new(tag.clone()).style(TextStyle::White))
